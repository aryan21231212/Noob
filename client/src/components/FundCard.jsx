...
const FundCard = ({ owner, title, description, target, deadline, amountCollected, image, handleClick, verified }) => {
  const remainingDays = daysLeft(deadline);

  return (
    <div
      className="sm:w-[288px] w-full rounded-[15px] bg-[#1c1c24] cursor-pointer hover:shadow-lg transition-shadow"
      onClick={handleClick}
    >
      ...
      <div className="flex flex-col p-4">
        ...
        <div className="block flex items-center gap-2">
          ...
          {verified && (
            <span className="px-2 py-1 text-xs bg-yellow-500 text-black rounded-full">Verified</span>
          )}
        </div>
